...
// mingw gives wrong value even though definitions we need are available,
// so forcibly override it
#undef _WIN32_WINNT
#define _WIN32_WINNT 0x0A00     // windows 10

//#define XSTR(x) STR(x)
//#define STR(x) #x
//#pragma message "Value of _WIN32_WINNT:" XSTR(_WIN32_WINNT)
...
struct Message
{
    ...
    void reply(const std::string& data)
    {
        ...
        ::send(fd, (const char*)&data[0], data.size(), 0);
    }
    ...
    int64_t read_int64()
    {
        ...
        int64_t result = 0;
        result |= (int64_t)bytes[parse_pos++] << 56;
        result |= (int64_t)bytes[parse_pos++] << 48;
        result |= (int64_t)bytes[parse_pos++] << 40;
        result |= (int64_t)bytes[parse_pos++] << 32;
        result |= (int64_t)bytes[parse_pos++] << 24;
        result |= (int64_t)bytes[parse_pos++] << 16;
        result |= (int64_t)bytes[parse_pos++] <<  8;
        result |= (int64_t)bytes[parse_pos++];
    ...
    uint64_t read_uint64()
    {
        ...
        uint64_t result = 0;
        result |= (uint64_t)bytes[parse_pos++] << 56;
        result |= (uint64_t)bytes[parse_pos++] << 48;
        result |= (uint64_t)bytes[parse_pos++] << 40;
        result |= (uint64_t)bytes[parse_pos++] << 32;
        result |= (uint64_t)bytes[parse_pos++] << 24;
        result |= (uint64_t)bytes[parse_pos++] << 16;
        result |= (uint64_t)bytes[parse_pos++] <<  8;
        result |= (uint64_t)bytes[parse_pos++];
...
class Server : public NetworkThread
{
  ...
  public:
    Server(unsigned short port = SENSEI_PORT) : NetworkThread(), port(port) {}
...
class Client : public NetworkThread
{
  ...
  public:    
    Client(std::string host, unsigned short port = SENSEI_PORT);
